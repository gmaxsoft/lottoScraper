...
import { launchHeadlessWithSession } from "./browser-session.js";
import type { DrawRecord } from "./types.js";
import { fetchProtocolSession } from "./tls-session.js";
import { humanMoveAndClick } from "./human-input.js";
import { trySolveTurnstileAny } from "./turnstile-solver.js";

/** Domyślna strona wyników (Nuxt) — spójna z wymaganiami hybrydy. */
const DEFAULT_RESULTS_URL = "https://www.lotto.pl/wyniki-gier";
...
/**
 * Hybryda: wreq-js (TLS chrome_124) → ciasteczka + UA → Chromium headless + opcjonalnie Capsolver/2Captcha na Turnstile.
 */
export async function scrapeLatestDraws(): Promise<DrawRecord[]> {
  const capturedJson: unknown[] = [];
  const resultsUrl =
    process.env.LOTTO_RESULTS_URL?.trim() || DEFAULT_RESULTS_URL;

  console.log("[hybryda] Krok 1: żądanie protokołowe (TLS Chrome 124) —", resultsUrl);
  const protocol = await fetchProtocolSession(resultsUrl);

  console.log(
    `[hybryda] Krok 2: Playwright headless z ${protocol.cookies.length} ciasteczkami (ten sam UA co wreq).`,
  );

  const { browser, page } = await launchHeadlessWithSession(
    protocol.cookies,
    protocol.userAgent,
  );
  ...
  try {
    ...
    if (await isHumanVerificationPresent(page)) {
      console.warn(
        "[turnstile] Wykryto wyzwanie — próba solvera (CAPSOLVER_API_KEY / TWOCAPTCHA_API_KEY).",
      );
      const solved = await trySolveTurnstileAny(page, page.url());
      if (!solved) {
        console.warn(
          "[turnstile] Solver nie zadziałał lub brak kluczy API. Uzupełnij CAPSOLVER_API_KEY lub TWOCAPTCHA_API_KEY w .env.",
        );
      } else {
        await page.waitForLoadState("networkidle", { timeout: 120_000 }).catch(() => {});
      }
